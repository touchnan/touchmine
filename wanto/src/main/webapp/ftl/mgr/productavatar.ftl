		...
		$('#file_upload').uploadify({
				...
				'onUploadSuccess' : function(file, data, response) {
					...
					if (r.obj && r.obj.success) {
						...
						$('#'+this.settings.queueID).append(imgsrc.format(showId, '${staticWebRoot}/'+r.obj.thumbnailRelativeFilename, showId));
		...
		$.ajax({
    		...
    		success: function(data){
    			if (preProcessData(data,function(msg){
    				...
    				window.location.href='${contextPath}/site/Topic-productview.htm?id=${topic.id?c}';
    			}
    		},
		...
		<div class="grid_16 bbs-form">
			...
			<table align="center">
				<form id = "dataForm">
				    ...
				    <tr>
				    	<td valign="top">
				    		<#if topic.thumbnail ?exists>
								<img src="${staticWebRoot}/${topic.thumbnail!''}"  alt="图片" style="float:left;margin:1px 18px 1px 0"/>
							</#if>
				    	</td>
				    	<td>
				    		<div>选择和您推荐的店铺相关的照片:</div>
				    		<br/>
							...
							<div id="queue"></div>
							<a style="dispaly:none;" id="picStartBtn" class="all-btn green-btn" href="javascript:upload()">上传</a> 
