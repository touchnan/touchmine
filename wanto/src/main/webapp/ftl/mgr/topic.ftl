  ...
  //翻页调用  
        function PageCallback(pageIndex, jq) {             
             $.ajax({   
                type: "POST",  
               dataType: "text",  
               url: '${contextPath}/c/Post-myPosts.htm',      //提交到一般处理程序请求数据  
                ...
                success: function(data) { 
                  ...
                  $("#Result tr:gt(0)").remove();          //移除Id为Result的表格里的行，从第二行开始（这里根据页面布局不同页变）  
                   for(var i=0 ;i< data.pageData.rows.length;i++){
                   		...
                   		'<td>'+'<a href=\"javascript:deletePost(\'' +rowData.id+    '\')\"' +'>删除</a></td>' +
                   		'</tr>');//将返回的数据追加到表格  
                  }
                }  
            });    
	...
	<div class="bbs-main">
		...
		<div class="right man-right">
			...
			<ul class="nav-module nav2">
				...
				<li class="active"><a href="${contextPath}/c/User-topic.htm"><span>我发起的帖子</span></a></li>
