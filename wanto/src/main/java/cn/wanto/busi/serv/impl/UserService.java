/*
 * cn.wanto.busi.serv.impl.UserService.java
 * May 25, 2012 
 */
...
public class UserService extends BaseService<UserDto, User, Long> implements IUserService {
     ...
     * (non-Javadoc)
     * 
     * @see cn.touchin.serv.BaseService#save(cn.touchin.dto.AppRequest, cn.touchin.dto.Dto)
     */
    @Override
     ...
     * (non-Javadoc)
     * 
     * @see cn.touchin.serv.BaseService#findPage(cn.touchin.dto.AppRequest, cn.touchin.page.Pagination)
     */
    @Override
     ...
     * (non-Javadoc)
     * 
     * @see cn.wanto.busi.serv.IUserService#findCacheUser(cn.wanto.dto.Ar)
     */
    @Override
    public List<UserDto> findCacheUser(Ar ar) {
        @SuppressWarnings("unchecked")
        List<User> entities =
                userDao.currSession().createCriteria(getEntityClass()).add(Restrictions.eq("hidden", true))
     ...
     * (non-Javadoc)
     * 
     * @see cn.wanto.busi.serv.IUserService#resetPasswd(java.lang.Long, java.lang.String, java.lang.String)
     */
    @Override
    @Transactional(propagation = Propagation.REQUIRED)
    public int resetPasswd(Long id, String oPasswd, String nPasswd) {
        oPasswd = oPasswd.trim();
     ...
     * (non-Javadoc)
     * 
     * @see cn.wanto.busi.serv.IUserService#resetSelfPasswd(cn.chuzheng.sgmr .dto.Ar, java.lang.String,
     * java.lang.String)
     */
    @Override
    @Transactional(propagation = Propagation.REQUIRED)
     ...
     * (non-Javadoc)
     * 
     * @see cn.wanto.busi.serv.IUserService#resetPasswd(cn.wanto. dto.Ar, java.util.List, java.lang.String)
     */
    @Override
    @Transactional(propagation = Propagation.REQUIRED)
     ...
     * (non-Javadoc)
     * 
     * @see cn.wanto.busi.serv.IUserService#auth(java.lang.String, java.lang.String)
     */
    @Override
    @Transactional(propagation = Propagation.REQUIRED)
    public Vu auth(String username, String passwd) {
        if (!Strings.isBlank(username) && !Strings.isBlank(passwd)) {
            User u =
                    userDao.fetch(null, new PropertyFilter("EQS_username", username), new PropertyFilter("EQS_passwd",
                            UserDto.encryptPasswd(passwd)));
            if (u != null) {
                return new Vu(u);
            }
        }
     ...
     * (non-Javadoc)
     * 
     * @see cn.wanto.busi.serv.IUserService#fetch(java.lang.String)
     */
    @Override
    public Vu auth(String username) {
        if (!Strings.isBlank(username)) {
            User u = userDao.fetch(null, new PropertyFilter("EQS_username", username));
            if (u != null) {
                return new Vu(u);
            }
        }
     ...
     * (non-Javadoc)
     * 
     * @see cn.wanto.busi.serv.IUserService#myInfo(cn.wanto.dto.Ar)
     */
    @Override
    public UserDto self(Ar ar) {
        UserDto dto = fetch(ar, ar.getLoginId());
        dto.setSchool(findWordbookNameById(dto.getSchoolId()));
        dto.setHometown(findWordbookNameById(dto.getHometownId()));
        return dto;
    }
     ...
     * @return
     */
    private String findWordbookNameById(Long id) {
        if (id == null || id == 0) {
            return "";
        }
        Wordbook o = dao().getHibernateTemplate().load(Wordbook.class, id);
     ...
     * (non-Javadoc)
     * 
     * @see cn.wanto.busi.serv.IUserService#emailExist(java.lang.String)
     */
    @Override
